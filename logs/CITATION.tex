...
for susceptibility distortions was estimated based on \emph{fMRIPrep}'s
\emph{fieldmap-less} approach. The deformation field is that resulting
from co-registering the EPI reference to the same-subject T1w-reference
with its intensity inverted \citep{fieldmapless1, fieldmapless2}.
Registration is performed with \texttt{antsRegistration} (ANTs 2.3.3),
...
fieldmap template \citep{fieldmapless3}.
